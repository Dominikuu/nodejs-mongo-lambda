...
resource "aws_iam_role" "KClambda_lambda_role" {
  ...
  assume_role_policy = "${data.aws_iam_policy_document.weather_lambda_document.json}"
}
resource "aws_lambda_permission" "KClambd_lambda_permission" {
  for_each = var.lambda_arn
